...
function PrismBlock({ language, code }) {
  ...
  return (
    <div className="relative my-3 rounded-lg overflow-hidden border border-[var(--border)] bg-[#1e1e2e]">
      <div className="flex items-center justify-between px-3 py-1.5 bg-[#2a2a3c] border-b border-[var(--border)]">
        <span className="text-[10px] font-mono uppercase tracking-wider text-gray-400">{lang}</span>
        <button
          onClick={handleCopy}
...
const markdownComponents = {
  code({ className, children, ...props }) {
    ...
    return (
      <code className="bg-[#2a2a3c] text-[#6BA3FF] px-1.5 py-0.5 rounded text-[12.5px] font-mono border border-[var(--border)]" {...props}>
        {children}
      </code>
    );
  },
...
function MessageBubble({ role, content, streaming }) {
  ...
  return (
    <div className={`flex gap-2.5 ${isUser ? 'flex-row-reverse' : ''}`}>
      <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${isUser ? 'bg-[var(--accent)]' : 'bg-[var(--bg-raised)] border border-[var(--border)]'}`}>
        {isUser ? <User className="w-3.5 h-3.5 text-white" /> : <Bot className="w-3.5 h-3.5 text-[var(--accent)]" />}
      </div>
      <div className={`max-w-[85%] rounded-2xl px-3.5 py-2 text-sm ${isUser ? 'bg-[var(--accent)] text-white rounded-tr-sm' : 'bg-[var(--bg-raised)] border border-[var(--border)] text-white rounded-tl-sm'}`}>
        {content ? (
      ...
      {isOpen && (
        <>
          ...
          <motion.aside
            ...
            className="fixed top-0 right-0 z-50 h-[100dvh] w-full max-w-md flex flex-col bg-[var(--bg-surface)] border-l border-[var(--border)] shadow-2xl"
          >
            {/* Header */}
            <header className="h-14 flex items-center justify-between px-4 border-b border-[var(--border)] shrink-0">
              <div className="flex items-center gap-2">
                <Sparkles className="w-4 h-4 text-[var(--accent)]" />
                <h2 className="text-sm font-display font-bold tracking-wide text-white uppercase">AI Sidecar</h2>
              </div>
              <div className="flex items-center gap-1">
                <button
                  onClick={clearAiHistory}
                  title="Clear AI history"
                  className="p-2 rounded-lg text-[var(--text-secondary)] hover:text-[var(--error)] transition-colors"
                >
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
              {aiMessages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center px-6 gap-3">
                  <div className="w-14 h-14 rounded-2xl bg-[var(--bg-raised)] border border-[var(--border)] flex items-center justify-center">
                    <Bot className="w-7 h-7 text-[var(--accent)]" />
                  </div>
                  <p className="text-sm text-white font-medium">Your private coding companion</p>
              ...
              {aiError && (
                <p className="text-[11px] text-[var(--error)] font-mono text-center">{aiError}</p>
              )}
            </div>

            {/* Composer */}
            <div className="p-3 border-t border-[var(--border)] shrink-0">
              <div className="flex items-end gap-2 bg-[var(--bg-base)] border border-[var(--border)] rounded-2xl p-1.5 focus-within:border-[var(--accent)] transition-colors">
                <textarea
                  ref={taRef}
                  value={inputDraft}
                  ...
                  className="flex-1 max-h-40 min-h-[40px] bg-transparent text-sm text-white resize-none focus:outline-none py-2 px-2 font-body"
                />
                {isStreaming ? (
                  <button
                    onClick={stopStreaming}
                    title="Stop generating"
                    className="w-10 h-10 rounded-full bg-[var(--bg-raised)] border border-[var(--border)] text-white flex items-center justify-center hover:border-[var(--error)] hover:text-[var(--error)] transition-all flex-shrink-0 active:scale-95"
                  >
                  ...
                  <button
                    ...
                    className="w-10 h-10 rounded-full bg-[var(--accent)] text-white flex items-center justify-center hover:bg-[var(--accent-hover)] transition-all disabled:opacity-40 disabled:cursor-not-allowed flex-shrink-0 active:scale-95"
                  >
